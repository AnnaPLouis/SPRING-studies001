...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> retrieveAccountsAgeOrderAsc();
    ...
    List<Account> retrieveAccountsAgeLessThan(int age);
    ...
    List<Account> retrieveAccountsContainingValue(@Param("value")String value);

    //Write a native query to read all accounts with an age higher than a specific value
    @Query(value = "SELECT * FROM account_details WHERE age > ?1",nativeQuery = true)
    List<Account> retrieveAccountsAgeGreaterThan(int age);
